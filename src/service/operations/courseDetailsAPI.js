...
export const addCourseDetails = async (data, token) => {
  ...
  try {
    ...
    result = response?.data?.course_info
  } catch (error) {
...
export const Publish=async(data,token)=>{
  const toastId = toast.loading("Loading...")
  let success = false
  try{
   const response=await apiConnector("PUT", PUBLISH_COURSE,data,
    {
      Authorization: `Bearer ${token}`, 
    }
   )
  ...
  toast.dismiss(toastId)
    ...
    success = false
    console.log("Publishing  API ERROR............", err)
    toast.dismiss(toastId)
